...
class MaterialQuestionController extends Controller
{
    public function index()
    {
        ...
        $materials = $allMaterials->map(function ($material) use (
            ...
            if ($isGuest) {
                ...
                $totalBeginners = $material->questions
                    ->where('difficulty', 'beginner')
                    ->count();
                $totalMediums = $material->questions
                    ->where('difficulty', 'medium')
                    ->count();
                $totalHard = $material->questions
                    ->where('difficulty', 'hard')
                    ->count();
                $jumlahTakeBeginners = (int) ceil($totalBeginners * 0.1);
                $jumlahTakeMediums = (int) ceil($totalMediums * 0.1);
                $jumlahTakeHard = (int) ceil($totalHard * 0.1);
                $beginnerCount = min(
                    $jumlahTakeBeginners,
                    $material->questions
                        ->where('difficulty', 'beginner')
                ...
                $hardCount = min(
                    $jumlahTakeHard,
                    $material->questions->where('difficulty', 'hard')->count()
                );
                $configuredTotalQuestions =
                    $beginnerCount + $mediumCount + $hardCount;
            } else {
                // Untuk pengguna terdaftar, gunakan konfigurasi admin
                $config = QuestionBankConfig::where(
                    'material_id',
                    $material->id
                )
            ...
            $materialProgress = $progressStats->firstWhere(
                'material_id',
                $material->id
            );
            $correctAnswers = $materialProgress
                ? $materialProgress->correct_answers
                : 0;
            ...
            $progressPercentage =
                $configuredTotalQuestions > 0
                    ? min(
                        100,
                        round(
                            ($correctAnswers / $configuredTotalQuestions) * 100
                        )
                    )
                    : 0;

            // Ambil jumlah mahasiswa yang sudah mencoba soal ini
            $studentCount = isset($studentCounts[$material->id])
                ? $studentCounts[$material->id]->student_count
                : 0;
    ...
    public function show(Material $material, Request $request)
    {
        ...
        if ($isGuest) {
            ...
            if ($difficulty === 'all') {
                // Modifikasi 10%
                $totalBeginners = $availableQuestions
                    ->where('difficulty', 'beginner')
                    ->count();
                $totalMediums = $availableQuestions
                    ->where('difficulty', 'medium')
                    ->count();
                $totalHard = $availableQuestions
                    ->where('difficulty', 'hard')
                    ->count();
                $jumlahTakeBeginners = (int) ceil($totalBeginners * 0.1);
                $jumlahTakeMediums = (int) ceil($totalMediums * 0.1);
                $jumlahTakeHard = (int) ceil($totalHard * 0.1);
                $beginnerQuestions = $availableQuestions
                    ->where('difficulty', 'beginner')
                    ->take($jumlahTakeBeginners);
                $mediumQuestions = $availableQuestions
                    ->where('difficulty', 'medium')
                    ->take($jumlahTakeMediums);
                $hardQuestions = $availableQuestions
                    ->where('difficulty', 'hard')
                    ->take($jumlahTakeHard);
            ...
            if ($config) {
                if ($difficulty === 'all') {
                    $beginnerQuestions = $availableQuestions
                        ->where('difficulty', 'beginner')
                        ->take($config->beginner_count);
                    $mediumQuestions = $availableQuestions
                        ->where('difficulty', 'medium')
                        ->take($config->medium_count);
                    $hardQuestions = $availableQuestions
                        ->where('difficulty', 'hard')
                        ->take($config->hard_count);
        ...
        if ($difficulty === 'all') {
            ...
            $beginnerQuestionsShuffled = $questions
                ->where('difficulty', 'beginner')
                ->shuffle();
            $mediumQuestionsShuffled = $questions
                ->where('difficulty', 'medium')
                ->shuffle();
            $hardQuestionsShuffled = $questions
                ->where('difficulty', 'hard')
                ->shuffle();

            // Gabungkan kembali dengan urutan beginner, medium, hard
            $questions = $beginnerQuestionsShuffled
                ->concat($mediumQuestionsShuffled)
                ->concat($hardQuestionsShuffled);
        } else {
        ...
        if (!$currentQuestion) {
            $currentQuestion = $questions
                ->reject(function ($question) use ($answeredQuestionIds) {
    ...
    public function levels(Material $material, Request $request)
    {
        ...
        foreach ($questions as $index => $question) {
            ...
            if ($isAnswered) {
                ...
                $index > 0 &&
                $answeredQuestionIds->contains($questions[$index - 1]->id)
            ) {
        ...
        return view(
            ...
            compact('material', 'materials', 'levels', 'difficulty', 'isGuest')
        );
    }
    ...
    public function review($id, Request $request)
    {
        ...
        if ($isGuest) {
            ...
            $answeredQuestionIds = collect(
                session('guest_progress.' . $material->id, [])
            );
        ...
        try {
            ...
            if (auth()->check() && auth()->user()->role_id !== 4) {
                ...
                if (!$existingCorrectProgress || !$isCorrect) {
                    ...
                    $attemptNumber =
                        $attemptsCount > 0 ? $attemptsCount + 1 : 1;
    ...
    public function showLevels(Material $material, Request $request)
    {
        ...
        if ($isGuest) {
            ...
            $jumlahTake = (int) ceil($jumlahSoal * 0.1);
            $questions = $questions->take($jumlahTake);
        } else {
            // Untuk user terdaftar, gunakan konfigurasi dari admin
            $config = QuestionBankConfig::where('material_id', $material->id)
                ->where('is_active', true)
    ...
    public function dashboard()
    {
        ...
        foreach ($allMaterials as $material) {
            if ($isGuest) {
                ...
                $configuredEasyQuestions += min(
                    3,
                    $material->questions
                        ->where('difficulty', 'beginner')
                        ->count()
                );
                $configuredMediumQuestions += min(
                    3,
                    $material->questions->where('difficulty', 'medium')->count()
                );
                $configuredHardQuestions += min(
                    3,
                    $material->questions->where('difficulty', 'hard')->count()
                );
            } else {
                ...
                if ($config) {
                    ...
                    $configuredEasyQuestions += $material->questions
                        ->where('difficulty', 'beginner')
                        ->count();
                    $configuredMediumQuestions += $material->questions
                        ->where('difficulty', 'medium')
                        ->count();
                    $configuredHardQuestions += $material->questions
                        ->where('difficulty', 'hard')
                        ->count();
                }
            }
        ...
        $configuredTotalQuestions =
            $configuredEasyQuestions +
            $configuredMediumQuestions +
            $configuredHardQuestions;
